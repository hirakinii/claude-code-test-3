...
- [x] Prismaマイグレーションが正常に実行でき、全テーブルが作成される
- [x] シードデータが正常に投入され、デフォルトスキーマとテストユーザーが利用可能
- [x] JWT認証が動作し、トークンの生成・検証が成功する
- [x] RBACミドルウェアが正しくロールを検証する
- [x] ヘルスチェックエンドポイントが正常に応答する
- [x] 全ての実装に対してテストが作成され、カバレッジ80%以上を達成
- [x] セキュリティチェックリストの全項目がクリアされる
