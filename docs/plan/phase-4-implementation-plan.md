...
- [ ] スキーマ定義から動的にフォームが生成される
- [ ] 全フィールドタイプ（TEXT, TEXTAREA, DATE, RADIO, CHECKBOX, LIST）が動作する
- [ ] ステップナビゲーション（次へ/前へ）が動作する
- [ ] 進捗バーが正しく表示される
- [ ] ローカル自動保存が動作する
- [ ] ページリロード時にデータが復元される
- [ ] 動的リスト（納品物、業務タスク等）の追加/削除が動作する
- [ ] 保存APIが正しく動作する
- [ ] バージョン管理が正しく動作する
